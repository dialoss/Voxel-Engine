...
void createChunks(Camera* &camera, ChunkManager* &world) {
	...
	while (!Window::shouldClose()) {
		...
		if (Config::curType != "struct" && Config::curType != "flat") 
...
void actions() {
	while (!Window::shouldClose()) {
		...
		for (int i = 0; i < 3; i++)
		{
			if (io.MouseDown[i]) Events::onMousePress(i, GLFW_PRESS);
			else Events::onMousePress(i, GLFW_RELEASE);
		}
		if (ImGui::IsKeyPressed(32)) Events::keyCallback(32, GLFW_PRESS);
		else Events::keyCallback(32, GLFW_RELEASE);

		for (int i = 0; i < 26; i++)
		{
			if (ImGui::IsKeyPressed('A' + i))
				Events::keyCallback('A' + i, GLFW_PRESS);
			else Events::keyCallback('A' + i, GLFW_RELEASE);
		}
	}
...
public:
	Game() {
		...
		NoiseGenerator::init(Config::BW * W, Config::BW * D, glm::vec2(0, 0), 1, 1);
	...
	void addChunks() {
		while (world->toAdd.size()) {
			...
			updateChunks();
			//RendererManager::buffers[0]->freeChunks[chunk->range].push_back(chunk->prevPos);
	...
	void unloadChunks() {
		while (world->toUnload.size()) {
			...
			world->toUnload.pop();
			if (chunk->removed) RendererManager::removeChunk(chunk, true);
			chunk->loaded = false;
			world->visibleSurfaces -= chunk->visibleSurfaces;
			world->waterSurfaces -= chunk->waterSurfaces;
		}
	...
	void run() {
		...
		std::thread proceedActions(std::ref(actions));
		...
		glm::vec2 translation(0, 0), prevtrans(0, 0);
		float noiseScale = 1.0f, noiseAmp = 1.0f;
		float prevsc = noiseScale, prevamp = noiseAmp;
		Texture* test_text = new Texture();
		...
		while (!Window::shouldClose()) {
			...
			if (prevtrans != translation || prevamp != noiseAmp || prevsc != noiseScale) {
				NoiseGenerator::init(Config::BW * W, Config::BW * D, translation, noiseScale, noiseAmp);
				world->reload();
				//test_text->loadFromBuffer(NoiseGenerator::heightMap, image_w, image_h, GL_RGBA);
			...
			{
				ImGui::Begin("image");
				ImGui::SliderFloat2("translation", &translation.x, -100.0f, 100.0f);
				ImGui::SliderFloat("scale", &noiseScale, 0.05f, 10.0f);
				ImGui::SliderFloat("amplitude", &noiseAmp, 0.05f, 2.0f);

				ImGui::Image((void*)(intptr_t)test_text->texture, ImVec2(image_w, image_h), ImVec2(1, 1), ImVec2(0, 0));
				ImGui::End();
			}
