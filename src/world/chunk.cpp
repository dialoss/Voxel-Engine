...
void Chunk::defaultWorld() {
	...
	for (uint x = 0; x < W; x++) {
		for (uint z = 0; z < D; z++) {
			...
			uint bottom = getHeight(realX, realZ, 1.2f) * 20;
			uint base = getHeight(realX, realZ, 0.5f) * 50;
			uint mount = getHeight(realX + 1000, realZ + 1000, 2) * 40;
			uint cliffs = getHeight(realX - 100, realZ + 100, 3) * 40;
			uint h = 0;
	...
	for (uint x = 0; x < W - (grid - 1); x+= grid) {
		for (uint z = 0; z < D - (grid - 1); z+= grid) {
			...
			float h11 = hValue(x1, z1);
			float h12 = hValue(x1, z2);
			float h21 = hValue(x2, z1);
			float h22 = hValue(x2, z2);
			...
			for (int i = 0; i < grid; i++)
			{
				for (int j = 0; j < grid; j++)
				{
					...
					int realX = this->posX * W + px;
					int realZ = this->posZ * D + pz;

					int h = bilinearInterp(h11, h12, h21, h22, x1, x2, z1, z2, px, pz);
					heightMap[pz + W * px] = h;
				}
			}	
		}
